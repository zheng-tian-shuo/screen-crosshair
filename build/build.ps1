...
Write-Host '[2/3] 编译'
$exe = Join-Path $buildDir '得吃准星 v6.3.exe'
...
Write-Host '[3/3] 整理输出文件'
# 文件名和窗口标题保持一致，都叫「得吃准星 v6.3」
$final = $exe

# Clean the duplicate generated by older versions of this script.
$legacyRootFinal = Join-Path $root '得吃准星 v6.1.exe'
if (Test-Path $legacyRootFinal)
{
    try { Remove-Item $legacyRootFinal -Force }
    catch { Write-Host ('  根目录旧包删不掉（可能还在运行）：' + $legacyRootFinal) }
}
# 改过名的旧包留着只会让人分不清哪个是新的（v1 的「屏幕准星.exe」不在这个清单里，不会被删）
$olds = @('得吃准星 v6.1.exe', '屏幕准星 v3.exe', '屏幕准星 v3.0.exe', '得吃准星 v3.0.exe', '得吃准星.exe')
foreach ($o in $olds)
{
    $p = Join-Path $buildDir $o
